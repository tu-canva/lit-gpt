...
def main(
    ...
    fabric.print(system_prompt.format(prompt="INSERT YOUR PROMPT HERE"))
    ...
    df000 = pd.read_csv(csv_path)
    if 'id' in df000:
        df000.drop_duplicates(['id'], inplace=True)
    results = []
    for prompt in tqdm(df000['input']):
        prompt = str(prompt)
        if to_lower:
            prompt = prompt.lower()
        ...
        generated_text = tokenizer.decode(y)[len(prompt):]
        results.append(generated_text)
        tokens_generated = y.size(0) - prompt_length
        # fabric.print(
        #     f"Inference time: {t:.02f} sec total, {tokens_generated / t:.02f} tokens/sec", file=sys.stderr
        # )
    df000['pred'] = results
    df000.to_csv(out_path, index=False)
